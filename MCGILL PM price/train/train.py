...
from model.loss import max_sharpe, equal_risk_parity, max_r2
...
class Trainer:
        ...
        for epoch in range(self.config["EPOCHS"]):
            ...
            for phase in ["train", "valid"]:
                ...
                for idx, data in enumerate(dataloader):
                    ...
                    with torch.set_grad_enabled(phase == "train"):
                        ...
                        if phase == "train":
                            ...
                            self.criterion(self.model(x), target).backward()
                            self.optimizer.second_step(zero_grad=True)

                    running_loss += loss.item() / len(dataloader)
                    running_r2 += r2.item() / len(dataloader)
                print(running_r2)
                ...
                if phase == "train":
                    ...
                    self.writer.add_scalar('Loss/eval', running_loss, epoch)
    ...
    def backtest(self, visualize=True):
        self.model = load_model(self.model, "result/best_model_weight_hb.pt", use_cuda=False)
